...
Update 14 July 2020
Addition of Dave McCabe's code to allow two inputs to be switched simultaneously.
...
const char string_16[] PROGMEM = "reset all servos";
...
const char string_65[] PROGMEM = "      v1.1";
...
const char string_70[] PROGMEM = "System settings";
...
int read_button()  
{
   ...
   if (analogValue < 40)                                                 {result=btnRIGHT;}  // 0    
   if ( (analogValue>50) && (analogValue < (down_val-50)) )              {result=btnUP;}     // 1    
   if ( (analogValue>(up_val+50))   && (analogValue < (left_val-50)) )   {result=btnDOWN;}   // 2    
   if ( (analogValue>(down_val+50)) && (analogValue < (select_val-50)) ) {result=btnLEFT;}   // 3    
   if ( (analogValue>(left_val+50)) && (analogValue<1000)  )             {result=btnSELECT;} // 4    
   if (analogValue > 1000)                                               {result = btnNONE;} // 5   
   return result;  }
//-------------------------------------------------------------------------
...
{                              // Reads all 16 pins (port A and B) into a single 16 bits variable
  ...
  Wire.requestFrom(addr, 2);  // read the current GPIO output latches (2 bytes)
  a  = Wire.read();
  ba = Wire.read();
  if (error == 0)
  {
  ...
  ba <<= 8;      // shift ba left 8 places
  ba |= a;       // Or ba with a
  return ba;     // ba stores 16 bits, one for the state of each input pin
}
//-------------------------------------------------------------------------
void send_command()
{  
...
void start_of_day()
{   
  for(byte servoModuleNum=0;servoModuleNum<16;servoModuleNum++)  // Look for each servo module
  { 
    if (bitRead(servoMap,servoModuleNum)==1)     // If the servo module exists
    {
      ...
      if (bitRead(switchMap,parent)==1)          // If the servo module has a matching switch module
       { 
         byte lookup = switchBaseAddress+parent;  
         ...
         for(byte pinOut = lb;pinOut < ub; pinOut++)
          {     
             ...
             Mem_Address = 32*servoModuleNum+4*PinNum;    
            ...
            previous_check[PinNum]= switchModuleState;         // value?
         }
       } 
    }  
  }   
...
void centre_servos()
{
   ...
   lcd.setCursor(0,1);lcdSay(16);         //"reset all servos" 
   delay(1000);
...
{
  serialSayln(70);                                                        // "System settings:"
...
//-------------------------------------------------------------------------
void SignalDown(byte up, byte down, byte range)
{ PinTemp = PinNum;
  lcd.clear();
...
{  
   ...
   do 
   {
      ...
      {                                                // Chosen to test servo speed
         ...
         int holdit = ((distance/servo_speed)*400)+ 2000; 
         delay(holdit);
         Wire.beginTransmission(TargetModule);          // transmiting to chosen module 
...
void pin_configuration()
{
  ...
  if (button_key == btnLEFT)
  {
     ...
     get_pin();     
     get_mode();        
     Mem_Address = 24*ModuleNum+4*PinNum;  
  ...
  {    
   if (bitRead(switchMap,switchNum)==1)                    // If the switch module exists                     
    {                                                  
      ...
      {
         diff = (previous_check[switchNum] ^ new_input);   // Calculate the difference from last time, using XOR
         ...
         if(mode==1)              // semaphore servo
           {
              byte ndiff=abs(lvalue-rvalue);        
              if (switch_state==1)
                {
                    SignalUp(lvalue,rvalue,ndiff);   
                }
              else
                {
                    SignalDown(lvalue,rvalue,ndiff);  
                }  
           }  
         ...
         if(mode==2)                                      //  Digital mode
           {                                                
              ...
              send_command();                             //  Send the message down the I2C bus
           }  
         previous_check[switchNum]= (diff & ~(1<<PinNum)) ^new_input;    
      }
    } 
  }
}
